...
class LavalinkMonitor {
    ...
    createAlertEmbed(eventType, node, error, additionalInfo, severity) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setTimestamp()
            .addFields('🖥️ Node', `\`${nodeId}\``, true)
            .addFields('🌐 Host', `\`${nodeHost}:${nodePort}\``, true)
            .addFields('📊 Severidade', `\`${severity.toUpperCase()}\``, true);

        // Adicionar informações do erro se disponível
        if (error) {
            embed.addFields('❗ Erro', `\`\`\`${error.message || error}\`\`\``, false);
        }

        // Adicionar informações adicionais se disponível
        if (additionalInfo) {
            embed.addFields('ℹ️ Informações Adicionais', additionalInfo, false);
        }
