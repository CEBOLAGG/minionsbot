...
for (const v of _dbList) {
    try {
        // Converte caminho absoluto antigo para relativo (compatibilidade cross-platform)
        let dbPath = v.path;
        const guildDbMatch = v.name.match(/^guild-(\d+)$/);
        if (guildDbMatch) {
            // Para guild databases, sempre recalcula o caminho baseado no diretório atual
            dbPath = join(__dirname, "..", ".guild_dbs", v.name + ".json");
        }
