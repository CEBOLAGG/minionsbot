...
async function handleWinnerList(interaction, giveawayData) {
    ...
    embed.addFields("Winners", winnersText);
...
async function handleWinnerAnnounce(interaction, giveawayData) {
    ...
    if (!channel.permissionsFor(interaction.guild.me).has(PermissionFlagsBits.SEND_MESSAGES) || 
        !channel.permissionsFor(interaction.guild.me).has(PermissionFlagsBits.EMBED_LINKS)) {
