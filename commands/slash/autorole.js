...
async function handleJoinAutorole(interaction, autoroleData) {
    ...
    if (autoroleData.joinRoles && autoroleData.joinRoles.length > 0) {
        ...
        embed.addFields({ name: "Currently Configured Roles", value: configuredRoles });
    } else {
        embed.addFields({ name: "Currently Configured Roles", value: "No roles configured" });
    }
    ...
    collector.on("collect", async i => {
        ...
        if (i.customId === "autorole_join_select") {
            ...
            if (configState.selectedRoles.length > 0) {
                ...
                updatedEmbed.addFields({ name: "Selected Roles", value: selectedRolesText });
            } else {
                updatedEmbed.addFields({ name: "Selected Roles", value: "No roles selected" });
            }
            ...
            if (success) {
                ...
                if (configState.selectedRoles.length > 0) {
                    ...
                    successEmbed.addFields({ name: "Configured Roles", value: selectedRolesText });
                } else {
                    successEmbed.addFields({ name: "Configured Roles", value: "No roles configured. New members will not receive automatic roles." });
                }
            ...
            const updatedEmbed = new EmbedBuilder()
                ...
                .addFields({ name: "Selected Roles", value: "No roles selected" });
    ...
    function updateConfigEmbed() {
        ...
        switch (configState.step) {
            case "message":
                ...
                embed.addFields({ name: "Next Step", value: "After defining the message, you will select the roles and emojis." });
                break;
            ...
            case "roles":
                ...
                if (configState.messageContent) {
                    embed.addFields({ 
                        name: "Defined Message", 
                        value: configState.messageContent.length > 1024 
                ...
                if (configState.roles.length > 0) {
                    ...
                    embed.addFields({ name: "Selected Roles", value: rolesText });
                }
                break;
            ...
            case "emojis":
                ...
                configState.roles.forEach((roleId, index) => {
                    ...
                    if (role) {
                        embed.addFields({ name: `Role #${index + 1}`, value: `${emoji} <@&${roleId}>` });
                    }
                });
                break;
            ...
            case "preview":
                ...
                embed.addFields({ name: "Channel", value: `<#${configState.channel.id}>` });
                ...
                if (configState.roles.length > 0) {
                    ...
                    embed.addFields({ name: "Roles and Emojis", value: rolesAndEmojis });
                }
                break;
        }
    ...
    componentCollector.on("collect", async i => {
        ...
        if (i.customId === "autorole_message_continue") {
            ...
            try {
                ...
                reactionEmbed.addFields({ name: "Available Roles", value: rolesAndEmojis });
...
async function handleListAutoroles(interaction, autoroleData) {
    ...
    if (autoroleData.joinRoles && autoroleData.joinRoles.length > 0) {
        ...
        embed.addFields({ name: "🔄 Join Autorole", value: `The following roles are automatically assigned to new members:\n${joinRoles}` });
    } else {
        embed.addFields({ name: "🔄 Join Autorole", value: "No join autorole configured." });
    }
    ...
    if (autoroleData.reactionRoles && autoroleData.reactionRoles.length > 0) {
        ...
        if (reactionRolesText) {
            embed.addFields({ name: "🎭 Reaction Systems", value: reactionRolesText });
        } else {
            embed.addFields({ name: "🎭 Reaction Systems", value: "No valid reaction system found." });
        }
    } else {
        embed.addFields({ name: "🎭 Reaction Systems", value: "No reaction system configured." });
    }
    ...
    collector.on("collect", async i => {
        ...
        if (i.customId === "autorole_remove_select") {
            ...
            if (selectedValue === "join") {
                confirmEmbed.addFields({ name: "Configuration to be removed", value: "Join Autorole" });
                ...
                confirmEmbed.addFields({ name: "Configuration to be removed", value: `Reaction System #${index + 1} in channel ${channel ? channel.name : "unknown"}` });
            }
