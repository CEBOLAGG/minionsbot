...
class DiscordMusicBot extends Client {
  ...
  async build() {
    ...
    // apiv2 (dashboard nova): REST + WebSocket em tempo real, ligado no ready.
    this.once("ready", () => {
      try {
        const ApiV2 = require("../apiv2");
        this.apiv2 = new ApiV2(this);
        this.apiv2.start();
      } catch (e) {
        this.error("[apiv2] init falhou: " + (e?.stack || e?.message || e));
      }
    });

    // Sync LEGADO pro MongoDB a cada 10s (método antigo). Desligado por padrão;
    // a dashboard nova recebe tudo ao vivo via WebSocket. Ligue com LEGACY_MONGO_SYNC=true.
    if (this.config.legacyMongoSync) {
      this.once("ready", () => {
        this.syncBotStats();
  ...
  createPlayerV2(guild, player, track, options = {}) {
    ...
    // Equalizador inline (toggle)
    const showEq = this.getPlayerData(guildId, "showEq") || false;
    ...
    // Enquanto o EQ está aberto, escondemos a linha secundária para não estourar o
    // limite de componentes do Components V2 (o EQ adiciona 3 linhas de botões).
    if (!showEq) mainContainer.addActionRowComponents(secondaryControls);

    // Linha do Equalizador (toggle inline)
    mainContainer.addActionRowComponents(
      new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          .setStyle(showEq ? ButtonStyle.Primary : ButtonStyle.Secondary)
          .setCustomId(`controller:${guildId}:ToggleEq`)
          .setEmoji(emojiFor("waves"))
          .setLabel("Equalizador")
      )
    );
    ...
    // Equalizador inline (toggle) — gráfico + ➕/➖ por banda + presets
    if (showEq) {
      try {
        require("../util/Equalizer").addInlineEq(mainContainer, this, guildId);
      } catch (e) {
        this.warn?.(`[eq] inline render: ${e.message}`);
      }
    }
