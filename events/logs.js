    ...
    async logModAction(guildId, action, moderator, target, reason, additionalInfo = {}) {
        try {
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: 'Moderator', value: `${moderator.tag} (${moderator.id})`, inline: true })
                .setTimestamp();
                
            if (target) {
                if (typeof target === 'object') {
                    embed.addFields({ name: 'Target', value: `${target.tag || target.user?.tag || 'Unknown'} (${target.id})`, inline: true });
                    ...
                    embed.addFields({ name: 'Target ID', value: target, inline: true });
                }
            }
    ...
    async logChannelCreate(channel) {
        ...
        try {
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: 'Type', value: this.getChannelType(channel.type) });
                
            if (channel.parent) {
                embed.addFields({ name: 'Category', value: channel.parent.name });
            }
    ...
    async logChannelUpdate(oldChannel, newChannel) {
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor('#FFFF00')
                .addFields({ name: 'Channel', value: `${newChannel.name} (${newChannel.id})` });
                
            if (oldChannel.name !== newChannel.name) {
                embed.addFields({ name: 'Name Changed', value: `${oldChannel.name} → ${newChannel.name}` });
            }
            
            if (oldChannel.type !== newChannel.type) {
                embed.addFields({ name: 'Type Changed', value: `${this.getChannelType(oldChannel.type)} → ${this.getChannelType(newChannel.type)}` });
            }
            ...
            if (oldChannel.parentId !== newChannel.parentId) {
                ...
                embed.addFields({ name: 'Category Changed', value: `${oldCategory} → ${newCategory}` });
            }
    ...
    async logRoleCreate(role) {
        try {
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: 'Role', value: `${role.name} (${role.id})` })
                .addFields({ name: 'Color', value: role.hexColor })
                .addFields({ name: 'Hoisted', value: role.hoist ? 'Yes' : 'No', inline: true })
                .addFields({ name: 'Mentionable', value: role.mentionable ? 'Yes' : 'No', inline: true })
                .addFields({ name: 'Position', value: role.position.toString(), inline: true });
    ...
    async logRoleUpdate(oldRole, newRole) {
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: 'Role', value: `${newRole.name} (${newRole.id})` });
            ...
            if (oldRole.hoist !== newRole.hoist) {
                embed.addFields({ name: 'Hoisted Changed', value: `${oldRole.hoist ? 'Yes' : 'No'} → ${newRole.hoist ? 'Yes' : 'No'}` });
            }
            
            if (oldRole.mentionable !== newRole.mentionable) {
                embed.addFields({ name: 'Mentionable Changed', value: `${oldRole.mentionable ? 'Yes' : 'No'} → ${newRole.mentionable ? 'Yes' : 'No'}` });
            }
    ...
    async logGuildUpdate(oldGuild, newGuild) {
        try {
            ...
            if (oldGuild.iconURL() !== newGuild.iconURL()) {
                embed.addFields({ name: 'Icon Changed', value: 'Server icon has been updated' });
                embed.setThumbnail(newGuild.iconURL({ dynamic: true }));
                changed = true;
            }
            
            if (oldGuild.bannerURL() !== newGuild.bannerURL()) {
                embed.addFields({ name: 'Banner Changed', value: 'Server banner has been updated' });
                changed = true;
            }
            ...
            if (oldGuild.description !== newGuild.description) {
                ...
                embed.addFields({ name: 'Description Changed', value: `${oldDesc} → ${newDesc}` });
                changed = true;
            }
            
            if (oldGuild.verificationLevel !== newGuild.verificationLevel) {
                embed.addFields({ name: 'Verification Level Changed', value: `${oldGuild.verificationLevel} → ${newGuild.verificationLevel}` });
                changed = true;
            }
    ...
    async logMemberLeave(member) {
        try {
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: 'Roles', value: member.roles.cache.size > 1 
        ...
        try {
            ...
            if (oldMember.premiumSince !== newMember.premiumSince) {
                if (newMember.premiumSince) {
                    embed.addFields({ name: 'Server Boost', value: `Started boosting <t:${Math.floor(newMember.premiumSince.getTime() / 1000)}:R>` });
                } else {
                    embed.addFields({ name: 'Server Boost', value: 'Stopped boosting' });
                }
            ...
            if (oldMember.pending !== newMember.pending) {
                embed.addFields({ name: 'Member Screening', value: newMember.pending ? 'Now pending' : 'Completed screening' });
                hasOtherChanges = true;
            }
    ...
    async logMemberBan(ban) {
        try {
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: 'Member', value: `${ban.user.tag} (${ban.user.id})` })
    ...
    async logMessageDelete(message) {
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: 'Channel', value: `${message.channel.name} (${message.channel.id})` })
    ...
    async logMessageUpdate(oldMessage, newMessage) {
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor('#FFFF00')
                .addFields({ name: 'Author', value: `${newMessage.author.tag} (${newMessage.author.id})` })
                .addFields({ name: 'Channel', value: `${newMessage.channel.name} (${newMessage.channel.id})` })
                .addFields({ name: 'Jump to Message', value: `[Click Here](${newMessage.url})` })
            ...
            if (oldMessage.content) {
                ...
                embed.addFields({ name: 'Before', value: content });
            }
            ...
            if (newMessage.content) {
                ...
                embed.addFields({ name: 'After', value: content });
            }
    ...
    async logMessageBulkDelete(messages) {
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor('#FF0000')
                .addFields({ name: 'Channel', value: `${messages.first().channel.name} (${messages.first().channel.id})` })
                .addFields({ name: 'Count', value: messages.size.toString() });
    ...
    async logVoiceStateUpdate(oldState, newState) {
        try {
            ...
            if (!oldState.channel && newState.channel) {
                const embed = new EmbedBuilder()
                    ...
                    .setColor('#00FF00')
                    .addFields({ name: 'Member', value: `${member.user.tag} (${member.id})` })
                    .addFields({ name: 'Channel', value: `${newState.channel.name} (${newState.channel.id})` })
                    .setThumbnail(member.user.displayAvatarURL({ dynamic: true }));
                ...
                if (oldState.channel && !newState.channel) {
                    const embed = new EmbedBuilder()
                        ...
                        .setColor('#FF0000')
                    .addFields({ name: 'Member', value: `${member.user.tag} (${member.id})` })
                    .addFields({ name: 'Channel', value: `${oldState.channel.name} (${oldState.channel.id})` })
                        .setThumbnail(member.user.displayAvatarURL({ dynamic: true }));
                ...
                if (oldState.channel && newState.channel && oldState.channel.id !== newState.channel.id) {
                    const embed = new EmbedBuilder()
                        ...
                        .setColor('#FFFF00')
                    .addFields({ name: 'Member', value: `${member.user.tag} (${member.id})` })
                    .addFields({ name: 'From', value: `${oldState.channel.name} (${oldState.channel.id})` })
                    .addFields({ name: 'To', value: `${newState.channel.name} (${newState.channel.id})` })
                        .setThumbnail(member.user.displayAvatarURL({ dynamic: true }));
            ...
            if (oldState.serverDeaf !== newState.serverDeaf && newState.channel) {
                    const embed = new EmbedBuilder()
        ...
        async logUserProfileUpdate(oldUser, newUser) {
            try {
                ...
                embed.addFields({ name: 'Username Changed', value: `${oldUser.tag} → ${newUser.tag}` });
                }
                
                if (oldUser.avatar !== newUser.avatar) {
                embed.addFields({ name: 'Avatar Changed', value: 'User changed their profile picture' });
                ...
                embed.addFields({ name: 'Banner Changed', value: 'User changed their profile banner' });
                }
        ...
        async logGuildMemberNicknameUpdate(oldMember, newMember) {
            try {
                ...
                .addFields({ name: 'Member', value: `${newMember.user.tag} (${newMember.user.id})` })
                .addFields({ name: 'Old Nickname', value: oldNick })
                .addFields({ name: 'New Nickname', value: newNick })
                    .setThumbnail(newMember.user.displayAvatarURL({ dynamic: true }));
        ...
        async logGuildMemberRoleAdd(member, role) {
            try {
                ...
                .addFields({ name: 'Member', value: `${member.user.tag} (${member.user.id})` })
                .addFields({ name: 'Role', value: `${role.name} (${role.id})` })
                    .setThumbnail(member.user.displayAvatarURL({ dynamic: true }));
